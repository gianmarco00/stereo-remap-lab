...
def test_right_coords_and_monotonicity() -> None:
    ...
    np.testing.assert_allclose(xr, np.array([-1.0, -0.2, 0.6, 1.2], dtype=np.float32), atol=1e-6)
...
def test_invert_monotonic_mapping_and_sampling() -> None:
    ...
    np.testing.assert_allclose(x_inv, np.array([0.0, 1.3333334, 2.6666667], dtype=np.float32), atol=1e-5)
    np.testing.assert_allclose(sampled, np.array([0.0, 13.333334, 26.666668], dtype=np.float32), atol=1e-4)
